...
int main(int argc, const char **argv)
{
    ...
    {
        int row_idx = i * y;
        int top_idx = min(max((i-1), 0), y) * y;
        int bot_idx = min(max((i+1), 0), y) * y;

        for (int j = 0; j < x; j++)
        {
            int left_ind = min(max((j-1), 0), x);
            int right_ind = min(max((j+1), 0), x);

            float top_left  = img[top_idx + left_ind];
            float top       = img[top_idx + j];
            float top_right = img[top_idx + right_ind];

            float left      = img[row_idx + left_ind];
            float right     = img[row_idx + right_ind];

            float bot_left  = img[bot_idx + left_ind];
            float bot       = img[bot_idx + j];
            float bot_right = img[bot_idx + right_ind];
